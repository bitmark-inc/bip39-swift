...
class BIP39Tests: XCTestCase {
    ...
    func testWordIndex() {
        ...
        for (index, mnemonic) in Mnemonics.enumerated() {
            print("test \(index): \(mnemonic)")
            // indexes to mnemonics
            XCTAssertEqual(BIP39Util.mnemonicFromWord(UInt16(index)), mnemonic)
            // mnemonics to indexes
            XCTAssertEqual(BIP39Util.wordFromMnemonic(mnemonic), Int16(index))
        }
    }
